...
| **TTS** | `TTS_ENABLED` | `false` | `/tts <text>` speaks into the channel via DuncteBot's `speak:` source. **Needs the DuncteBot plugin, which is currently disabled** (see below) — so `/tts` replies "disabled" until that's resolved and the flag is flipped. |
...
|---|---|---|
| **DuncteBot (skybot)** | `com.dunctebot:skybot-lavalink-plugin:1.7.1` | Would add extra sources + the free `speak:` TTS behind `/tts`, but the 1.7.1 jar **isn't self-contained**: it needs a separate `com.dunctebot:sourcemanagers` library on the classpath, which Lavalink's plugin loader doesn't fetch — so it threw `NoClassDefFoundError` (GetyarnAudioSourceManager) and **crash-looped the node**. Re-enable only with a self-contained build or after also providing the sourcemanagers jar. |
...
2. **Confirm**, then flip one flag at a time in `.env` and **restart the bot**:
   ...
   - `TTS_ENABLED=true` — only after a working DuncteBot build is uncommented
     (see the disabled-plugins note above); otherwise `/tts` replies "disabled".
